...
1. **Apache Kafka** — message broker для потоковой передачи данных (топики: `transactions`, `scores`, `teta_transactions`)
2. **Transaction Producer** — читает CSV и стримит транзакции в Kafka
3. **Fraud Scoring Service** — сервис ML-инференса с CatBoost моделью
4. **PostgreSQL** — хранилище результатов скоринга
5. **ClickHouse** — Kafka Engine + MV → MergeTree для аналитических запросов по транзакциям
6. **Streamlit Dashboard** — веб-интерфейс для мониторинга фрода
...
| `clickhouse`         | Принимает поток `teta_transactions` из Kafka и складывает его в MergeTree      |
...
## Kafka → ClickHouse (домашнее задание семинара 7)

### Что реализовано
- Скрипт загрузки CSV в Kafka: `clickhouse/load_to_kafka.py`.
- DDL для базовой загрузки Kafka → ClickHouse: `clickhouse/01_tables_from_kafka.sql`.
- Оптимизированная схема хранения (LowCardinality, Decimal, партиционирование, индекс по amount): `clickhouse/02_tables_optimized.sql`.
- Запрос «категория наибольшей транзакции по каждому штату»: `clickhouse/03_query_top_category.sql`.
- Пример результата на датасете `input/test.csv`: `output/us_state_top_category.csv`.

### Как повторить
1) Скачать `train.csv` TETA ML 1 2025 с Kaggle и положить в `input/train.csv` (можно использовать `input/test.csv` для быстрого прогона).

2) Поднять Kafka и ClickHouse:
```bash
docker compose up -d kafka clickhouse
```

3) Загрузить CSV в Kafka (по умолчанию в топик `teta_transactions`, без заголовка — формат в DDL `CSV`):
```bash
python3 clickhouse/load_to_kafka.py \
  --csv-path input/train.csv \
  --bootstrap localhost:9092 \
  --topic teta_transactions \
  --batch-size 5000
```

4) Создать таблицы в ClickHouse:
- базовая схема:
```bash
docker compose exec clickhouse clickhouse-client \
  -u click --password click --multiquery < clickhouse/01_tables_from_kafka.sql
```
- оптимизированная схема (повторяет таблицы с более компактными типами и индексом):
```bash
docker compose exec clickhouse clickhouse-client \
  -u click --password click --multiquery < clickhouse/02_tables_optimized.sql
```

5) Выполнить запрос по штатам и выгрузить результат в CSV:
```bash
docker compose exec clickhouse clickhouse-client \
  -u click --password click --query "$(cat clickhouse/03_query_top_category.sql)" \
  --format CSVWithNames > output/us_state_top_category.csv
```

После шага 5 в `output/us_state_top_category.csv` будут строки вида `us_state,cat_id,max_amount`. Для быстрой проверки без ClickHouse пример результата уже сохранён на основе `input/test.csv`.
