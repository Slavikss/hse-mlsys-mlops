...
3. **Data Analyst** — выполняет ad-hoc запросы по полному потоку транзакций в ClickHouse.
...
4. **ClickHouse (аналитика)** — принимает поток транзакций из Kafka и используется аналитиками для выборок (например, максимальная транзакция по штатам).
...
- Отдельный поток `teta_transactions` из Kafka пишется в ClickHouse через Kafka Engine + Materialized View для ad-hoc аналитики.
...
#### 4. ClickHouse Ingestion (Kafka → ClickHouse)

**Назначение**: Загружает сырой поток транзакций (например, `train.csv`/`test.csv` из задания) из Kafka в OLAP-хранилище для аналитики.

**Компоненты**:
- **Kafka Engine Table** — читает топик `teta_transactions` в формате CSV (без заголовка).
- **Materialized View** — приводит типы и парсит даты.
- **MergeTree Storage** — долговременное хранение с LowCardinality/Decimal, партиционированием и индексом по amount.

**Технологии**: ClickHouse (DDL: `clickhouse/02_tables_optimized.sql`)

**Типовые запросы**:
- `clickhouse/03_query_top_category.sql` — категория с максимальной транзакцией по каждому штату.

---

#### 5. Web UI (Streamlit Dashboard)
...
**Роль**: OLAP-хранилище для полного потока транзакций и запросов аналитиков.

**Конфигурация**:
- **База данных**: `fraud_demo`
- **Таблицы**:
  - `teta_transactions_kafka` (Kafka Engine, CSV, без заголовка, group `ck_teta_ingest_v3`)
  - `teta_transactions_raw` (ReplacingMergeTree с LowCardinality/Decimal, TTL 10 лет)
  - `mv_teta_transactions` (materialized view Kafka → MergeTree)
- **Индексы**: minmax по `amount`, порядок `(us_state, cat_id, transaction_time)`

**Запуск**: сервис `clickhouse` в `docker-compose.yml`, SQL-инициализация из каталога `clickhouse/`.
...
CSV File → Kafka (teta_transactions) → ClickHouse Kafka Engine → Materialized View → MergeTree → Data Analyst (SQL)
```
...
5. Параллельно топик `teta_transactions` читается ClickHouse (Kafka Engine → MV) и складывается в MergeTree для запросов аналитиков.
...
- ClickHouse — для аналитических запросов и разовых выгрузок
